...
Scikit-Learn interface object from 0.90 to 1.0.0 .  Pickle
...
def xgboost_skl_90to100(old_model):
    '''Extract the model and related metadata in old model.'''
    model = {}
    with open("underwirting_post_pipe.pkl", 'rb') as fd:
        old = joblib.load(fd)
        if not isinstance(old, xgboost.):
            raise TypeError(
...
# Save Scikit-Learn specific Python attributes into a JSON document.
    ...
    # Save it into a native model.(1.0.0)
    i = 0
    while True:
        path = 'xgboost_native_model_from_' + "underwirting_post_pipe" + '-' + str(i) + '.bin'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'underwirting_post_pipe.pkl',      #old model name
        type=str,
